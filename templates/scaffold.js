...
const pages = await new List({
  ...
  .then((pageNames) => pageNames.map((pageName) => _.kebabCase(pageName.toLowerCase())))
  .catch(console.error);

pages.forEach((page) => {
  fse.copy('./templates/page.html', `./src/${page}.html`)
    .catch((err) => console.error(err));
  fse.copy('./templates/page.ts', `./src/scripts/${page}.ts`, (err) => {
...
const elements = await new List({
  ...
  .then((elementNames) => elementNames.map((elementName) => elementName.toLowerCase()))
  .catch(console.error);
...
const sections = await new List({
  ...
  .then((sectionNames) => sectionNames.map((sectionName) => sectionName.toLowerCase()))
  .catch(console.error);
